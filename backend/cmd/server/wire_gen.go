...
func initializeApplication(buildInfo handler.BuildInfo) (*Application, error) {
	...
	apiKeyService := service.ProvideAPIKeyService(apiKeyRepository, userRepository, groupRepository, userSubscriptionRepository, userGroupRateRepository, apiKeyCache, configConfig, billingCacheService, serviceUserPlatformQuotaRepository)
	...
	promptAuditService := service.NewPromptAuditService(settingRepository, promptAuditRepository)
	promptAuditHandler := admin.NewPromptAuditHandler(promptAuditService)
	...
	hvoyPricingService := service.NewHvoyPricingService(pricingDisplayService, groupRepository, settingService)
	hvoyPricingHandler := handler.NewHvoyPricingHandler(hvoyPricingService)
	idempotencyCoordinator := service.ProvideIdempotencyCoordinator(idempotencyRepository, configConfig)
	idempotencyCleanupService := service.ProvideIdempotencyCleanupService(idempotencyRepository, configConfig)
	handlers := handler.ProvideHandlers(authHandler, userHandler, apiKeyHandler, usageHandler, redeemHandler, subscriptionHandler, announcementHandler, channelMonitorUserHandler, adminHandlers, gatewayHandler, openAIGatewayHandler, handlerSettingHandler, totpHandler, handlerPaymentHandler, paymentWebhookHandler, availableChannelHandler, publicPricingHandler, pricingDisplayHandler, hvoyPricingHandler, feedbackHandler, accountAPIHandler, providerConnectHandler, idempotencyCoordinator, idempotencyCleanupService)
