...
func (h *GatewayHandler) GeminiV1BetaModels(c *gin.Context) {
	...
	slotUserID, slotPlatform, slotMax := middleware.ResolveUserSlot(c)
	userReleaseFunc, err := geminiConcurrency.AcquireUserSlotWithWait(c, slotUserID, slotPlatform, slotMax, stream, &streamStarted)
	if err != nil {
