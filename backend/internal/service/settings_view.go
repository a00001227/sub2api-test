...
type SystemSettings struct {
	...
	// 客户密钥强制通道前缀（默认 false → 裸路径按主通道计费）
	SubKeyChannelPrefixRequired bool
	...
	EnableClaudeOAuthTemperatureInjection  bool   // 是否对 Claude OAuth mimic 路径注入 temperature=1（默认 true=旧行为）；关闭后剥离 temperature，避免"已弃用 temperature 的模型"报 400
