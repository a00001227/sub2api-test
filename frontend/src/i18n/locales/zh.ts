  ...
  admin: {
    ...
    settings: {
      ...
      site: {
        ...
        customEndpoints: {
          title: '自定义端点',
          description: '添加额外的 API 端点地址，用户可在「API Keys」页面快速复制。名称/介绍支持「中文 | English」双语（用 | 分隔，前中后英，按界面语言显示）；端点地址填原始 URL，不要加 |。',
          itemLabel: '端点 #{n}',
          name: '名称',
          namePlaceholder: '中文名称 | English name（如：OpenAI 兼容 | OpenAI Compatible）',
          endpointUrl: '端点地址',
          endpointUrlPlaceholder: 'https://api2.example.com',
          descriptionLabel: '介绍',
          descriptionPlaceholder: '中文介绍 | English desc（如：支持 OpenAI 格式 | OpenAI-compatible）',
          add: '添加端点',
        },
        contactInfo: '客服联系方式',
        contactInfoPlaceholder: 'Telegram群链接 | 邮箱 | Telegram客服 | WhatsApp',
        contactInfoHint: '4 段固定顺序、用 | 分隔：① Telegram 群链接 ② 邮箱 ③ Telegram 客服 ④ WhatsApp。某项没有就留空但保留 |（如「| a@b.com | @cs |」），第 5 段及以后会被忽略。展示在兑换页面、个人资料等位置。',
        docUrl: '文档链接',
