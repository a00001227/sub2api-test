export default {
  ...
  admin: {
    ...
    risk: {
      title: '风险监控',
      description: '反蒸馏影子看板：观测高风险用户的评分与特征，当前不拦截任何请求。',
      refresh: '刷新',
      loadFail: '加载风险数据失败',
      // Shadow-mode banner
      shadowBannerTitle: '影子模式 · 仅观测，不拦截任何请求',
      shadowBannerDesc: '当前处于 Phase 0 观测阶段，评分与"若开拦会做"仅供参考，不会对任何用户生效。',
      enforceBannerTitle: '强制模式已开启 · 正在对真实请求执行动作',
      enforceBannerDesc: '风控动作已生效，会影响真实用户流量，请谨慎操作。',
      currentMode: '当前模式',
      modeObserve: '观测',
      modeEnforce: '强制',
      // Tier filter
      ...
      wouldNone: '无动作',
      manualAuto: '自动',
      manualTierUpdated: '手动等级已更新',
      manualTierFail: '更新手动等级失败',
      allowlistOn: '已加入白名单',
      allowlistOff: '已移出白名单',
      allowlistFail: '更新白名单失败',
      // Empty state
      emptyTitle: '暂无评分数据',
      emptyDesc: '暂无评分数据，worker 每 ~5 分钟聚合一次。',
      // Row detail
      detailTitle: '用户风险详情',
      close: '关闭',
      featuresSection: '特征向量 (f1–f7)',
      windowSection: '24 小时窗口',
      f1: 'f1 多样性坍缩',
      f2: 'f2 单轮占比',
      f3: 'f3 输出榨取',
      f4: 'f4 机械节奏',
      ...
      f7: 'f7 扇出新颖度',
      winRequests: '请求数',
      winOutputTokens: '输出 token',
      winDistinctRatio: '去重比',
      winSingleTurn: '单轮比',
      winTopModel: '主模型',
      winRpmPeak: 'RPM 峰值',
      winBudgetPct: '预算占比',
      detailLoadFail: '加载用户详情失败',
      // Config panel
      configTitle: '风控配置',
      configHint: 'Phase 0 配置存于内存，服务重启后会重置为默认值。',
      configMode: '模式',
      configModeObserveOpt: '观测（observe）',
      configModeEnforceOpt: '强制（enforce）',
      enforceWarning: '⚠ 切换到"强制"会开启真实拦截动作，影响线上用户，请谨慎。默认应保持"观测"。',
      mediumScore: '中危阈值分',
      highScore: '高危阈值分',
      volumeFloor: '最低流量门槛',
      andGateK: 'AND 门 K 值',
      dailyBudget: '每日预算 (USDC)',
      weeklyBudget: '每周预算 (USDC)',
      weightsSection: '特征权重 (f1–f7)',
      thresholdsSection: '特征阈值 (f1–f7)',
      save: '保存配置',
      saving: '保存中...',
      saveSuccess: '风控配置已保存',
      saveFail: '保存风控配置失败',
      configLoadFail: '加载风控配置失败',
      expand: '展开',
      collapse: '收起',
    }
  },
